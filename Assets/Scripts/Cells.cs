    ...
    public void SpawnParticle(int currentPositionX,int currentPositionY)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            for (int j = 0; j < 5; j++)
            {
                ...
                if((Mathf.Abs(currentPositionX-i) <= 1 && Mathf.Abs(currentPositionY-j) == 0) || (Mathf.Abs(currentPositionX - i) == 0 && Mathf.Abs(currentPositionY - j) <= 1) || (Mathf.Abs(currentPositionX - i) <= 1 && Mathf.Abs(currentPositionY - j) <= 1))
                {
                    newParticle = null;
                    bool spawn = false;
